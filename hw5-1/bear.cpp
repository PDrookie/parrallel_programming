...
int main()
{
    ...
    #pragma omp parallel for shared(global_min_weighted_cost)
    for(int i = 0;i < all_permutation_list.size();i++){  // parallelize this for loop 
        ...
        // refresh the min
        #pragma omp critical // avoid data race
